...
const int nax = 305;
double cache[nax][nax][nax];
bool vis[nax][nax][nax];
int cnt[4];
int n;
int d[nax];
// cache[a][b][c] - the expected number operation so far to get the state a , b , c
...
double dp(int a , int b , int c){
	...
	if(a == 0 && b == 0 && c == 0)
		return 0;
	double &ans = cache[a][b][c];
	if(vis[a][b][c])
		return ans;
	vis[a][b][c] = 1;
	ans = n + a*dp(a-1,b,c) + b*dp(a+1,b-1,c) + c*dp(a,b+1,c-1);
	ans/=(a+b+c);
	return ans;
}
...
struct Solver {
 void solve(){
	...
	double ans = dp(cnt[1],cnt[2],cnt[3]);
	cout<<ans<<"\n";
  }
};
