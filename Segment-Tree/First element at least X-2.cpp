//@author : Mehul Jain
// first element greater equal to x
#include <bits/stdc++.h>
using namespace std;
...
struct segtree {
	...
	item first_above(item v ,item l, item x , item lx , item rx){
		if(values[x] < v) return -1;
		if(rx <= l) return -1;
		if(rx-lx == 1)
			return lx;
		item m = (lx + rx)/2;
		item res = first_above(v,l,2*x+1,lx,m);
		if(res == -1){
	...
	item first_above(item v,item l){
